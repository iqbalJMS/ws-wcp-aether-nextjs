...
export async function API_GetSimulationBriguna(request: T_SimulationBrigunaRequest) {
  try {
    let requestKarya: T_SimulationBrigunaRequest = {...request, type: 'KARYA'}
    const formDataKarya = new FormData();
    Object.entries(requestKarya).forEach(([key, value]) => {
      formDataKarya.append(key, value.toString());
    });
    const responseKarya = await post<T_PostResponse<T_SimulationBriguna[]>>('/api/brimw/simulasi/estimateBriguna', formDataKarya);
    let requestPurna: T_SimulationBrigunaRequest = {...request, type: 'PURNA'}
    const formDataPurna = new FormData();
    Object.entries(requestPurna).forEach(([key, value]) => {
      formDataPurna.append(key, value.toString());
    });
    const responsePurna = await post<T_PostResponse<T_SimulationBriguna[]>>('/api/brimw/simulasi/estimateBriguna', formDataPurna);
